...
void all_data_update(void)
{
	...
	mcu_dp_enum_update(DPID_BATTERY_STATE,0); //当前电池电量状态枚举型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_SAMPLING,2); //当前湿度采样时间设置VALUE型数据上报;
    mcu_dp_enum_update(DPID_TEMP_UNIT_CONVERT,0); //当前温标切换枚举型数据上报;
    mcu_dp_value_update(DPID_MAXTEMP_SET,100); //当前温度上限设置VALUE型数据上报;
    mcu_dp_value_update(DPID_MINITEMP_SET,550); //当前温度下限设置VALUE型数据上报;
    mcu_dp_value_update(DPID_MAXHUM_SET,0); //当前湿度上限设置VALUE型数据上报;
    mcu_dp_value_update(DPID_MINIHUM_SET,95); //当前湿度下限设置VALUE型数据上报;
    mcu_dp_value_update(DPID_TEMP_PERIODIC_REPORT,2); //当前温度周期上报VALUE型数据上报;
    mcu_dp_value_update(DPID_HUM_PERIODIC_REPORT,2); //当前湿度周期上报VALUE型数据上报;
    mcu_dp_bool_update(DPID_SWITCH,0); //当前开关BOOL型数据上报;
    mcu_dp_value_update(DPID_TEMP_SET,150); //当前温度设置VALUE型数据上报;
...
static unsigned char dp_download_humidity_sampling_handle(const unsigned char value[], unsigned short length)
{
    ...
    */
    temp_sampling_period = humidity_sampling;
    //处理完DP数据后应有反馈
...
static unsigned char dp_download_switch_handle(const unsigned char value[], unsigned short length)
{
    ...
    if(switch_1 == 0) {
       HAL_GPIO_WritePin(RL1_GPIO_Port,RL1_Pin,GPIO_PIN_RESET); //开关关
    }else {
       HAL_GPIO_WritePin(RL1_GPIO_Port,RL1_Pin,GPIO_PIN_SET);//开关开
    }
